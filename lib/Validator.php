...
class Validator
{
	...
	public function isUnique($value, $parameter)
	{
		$db = Db::init();
		$collection = $db->{$parameter['table']};
		$icol = $collection->count(array($parameter['field'] => $value));
		if($icol > 0)
			return false;
		return true;
	}
    ...
    protected function check($value, $rule, $parameter)
    {
        ...
        {
        	case 'unique' :
				return $this->isUnique($value, $parameter);
            case 'require' :
                return!(trim($value) == '');
    ...
    protected function setDefaultMessages()
    {
        $this->_messages = array(
                ...
                'regex' => 'Invalid value.',
                'unique' => 'Data Sudah Ada silahkan isi dengan yang lain.',
        );
    }
}